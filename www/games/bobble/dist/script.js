...
var xs = [[-193, -138, -83, -28, 27, 82, 137, 192], [-165, -110, -55, 0, 55, 110, 165]];
...
forever(function () {
	if (!frozen) {
		if (keysDown.includes('left')) arrow.turnLeft(2);
		if (keysDown.includes('right')) arrow.turnRight(2);
		if (arrow.angle >= 175) arrow.angle = 174;
		if (arrow.angle <= 5) arrow.angle = 6;
	}
});
...
onKeyDown(function () {
	if (keysDown.includes('space') && !shooting && !frozen) {
		...
		repeatUntil(function () {
			...
			// stop at the top
			if (playerBubble.y >= 212) touching = true;
		...
		function () {
			...
			if (topRowEight) {
				if (ys.indexOf(playerBubble.y) % 2) playerBubble.x = closestCoordinate(xs[1], playerBubble.x);else playerBubble.x = closestCoordinate(xs[0], playerBubble.x);
			}

			// if the top row has seven bubbles
			else {
					if (ys.indexOf(playerBubble.y) % 2) playerBubble.x = closestCoordinate(xs[0], playerBubble.x);else playerBubble.x = closestCoordinate(xs[1], playerBubble.x);
				}
			...
			if (matches.length > 2) {
				matches.forEach(function (bubble) {
					...
					var newScore = (currentScore + 23) + '', pad = '';
					
					for(var i=newScore.length; i<6; i++){
						pad += '0';
					}
...
function initialize() {
	...
	for (var j = 0; j < 4; j++) {
		...
		bubbleY -= 48;
		if (bubbleX === 247) bubbleX = -165;else bubbleX = -193;
	}
...
function dropDisconnectedBubbles() {
	...
	bubbles.forEach(function (bubble) {
		if (!bubble.connected) {
			...
			var currentScore = parseInt($('#score span').text());
			var newScore = (currentScore + 67) + '', pad = '';
...
function closestCoordinate(array, currentCoordinate) {
	return array.reduce(function (min, cur) {
		if (Math.abs(cur - currentCoordinate) < Math.abs(min - currentCoordinate)) return cur;
		return min;
	});
}
...
function markAllConnected(topBubble) {
	topBubble.touchingBubbles.forEach(function (bubble) {
		if (!bubble.connected) {
			bubble.connected = true;
